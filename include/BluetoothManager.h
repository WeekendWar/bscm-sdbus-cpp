...
public:
    ...
    std::vector<CharacteristicInfo> getCharacteristics(const std::string& servicePath);
    ...
    std::vector<uint8_t> readCharacteristic(const std::string& characteristicPath);
...
private:
    ...
    std::map<std::string, std::function<void(const std::vector<uint8_t>&)>> m_notifyCallbacks;
    
    std::string findAdapter();
    std::string getDevicePath(const std::string& address);
    std::map<std::string, sdbus::Variant> getProperties(const std::string& objectPath,
